...
# ==============================
# VERSION 7.2 FEATURES (NEW):
# 🔄 Recursive Missing File Scanner (scan entire folder hierarchies)
# Aa Global Case Sensitivity Toggle (affects all operations)
# 🗑️ Send to Recycle Bin (safe file removal with system integration)
# 🔒 Enhanced Security (user data protection)
#
# VERSION 7.0 FEATURES (PRESERVED):
...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        "case_sensitivity": {
            "enabled": False  # Default: case-insensitive for backward compatibility
        }
    }
...
# Case Sensitivity Toggle (v7.2)
case_sensitive_var = tk.BooleanVar(value=CONFIG.get("case_sensitivity.enabled", False))

def on_case_sensitivity_change():
    """Save case sensitivity preference to config"""
    CONFIG.set("case_sensitivity.enabled", case_sensitive_var.get())
    APP_LOGGER.info(f"Case sensitivity {'enabled' if case_sensitive_var.get() else 'disabled'}")

ttk.Checkbutton(
    paths_box,
    text="Aa Case-sensitive matching (affects all operations)",
    variable=case_sensitive_var,
    command=on_case_sensitivity_change
).grid(row=3, column=0, columnspan=3, sticky="w", padx=(8,6), pady=(0,8))
...
def is_case_sensitive() -> bool:
    ...
    Returns cached value from CONFIG.
    This is the single source of truth for case sensitivity across the app.

    EXCEPTION: Windows path validation always uses case-insensitive comparison
    for safety, regardless of this setting.

    Returns:
        bool: True if case-sensitive matching is enabled, False otherwise
    """
    return CONFIG.get("case_sensitivity.enabled", False)

def normalize_case(text: str, force_insensitive: bool = False) -> str:
    """
    Normalize text case based on current case sensitivity setting.

    Args:
        text: Text to normalize
        force_insensitive: If True, always use case-insensitive (for safety checks)

    Returns:
        Original text if case-sensitive mode enabled, lowercase otherwise
    """
    if force_insensitive or not is_case_sensitive():
        return text.lower()
    return text
...
def get_file_datetime(filepath: str) -> Optional[datetime]:
    ...
    try:
        ...
        if is_case_sensitive():
            ...
            matches = any(filepath.endswith(ext) for ext in extensions)
            matches = matches or any(filepath.endswith(ext.upper()) for ext in extensions)
        else:
            matches = filepath.lower().endswith(extensions)
...
def extract_img_tag(filename: str) -> Optional[str]:
    ...
    m = re.search(r"(IMG|DSC|DSCN|DCS|DCSN)(?=\d|_|\.|$)", filename, flags)
    if m:
        tag = m.group(1) if is_case_sensitive() else m.group(1).upper()
...
# ==============================
# RECURSIVE MISSING FILE SCANNER (v7.2)
# ==============================
def detect_file_patterns_recursive(parent_dir: str, progress_callback: Optional[Callable] = None) -> Dict[str, Dict]:
    """
    Recursively scan all subfolders and detect patterns in each independently.

    Args:
        parent_dir: Parent directory to scan
        progress_callback: Optional callback(current_folder, total_folders, folder_path)

    Returns:
        Dict mapping subfolder_path to pattern detection results:
        {
            'L:\\path\\to\\subfolder': {
                'folder_name': 'subfolder',
                'folder_path': 'L:\\path\\to\\subfolder',
                'patterns': {
                    'PURE_NUMERIC_.jpg': {...},
                    'IMG_': {...}
                },
    ...
    # First, collect all subdirectories
    try:
        ...
        APP_LOGGER.error(f"Error walking directory tree: {e}")
        return {}

    total_folders = len(all_subdirs)

    # Scan each directory independently
    for idx, dirpath in enumerate(all_subdirs):
        if progress_callback:
            try:
                progress_callback(idx + 1, total_folders, dirpath)
            except Exception as e:
                APP_LOGGER.error(f"Error in progress callback: {e}")

        try:
            # Detect patterns in this specific directory
            patterns = detect_file_patterns(dirpath)

            if patterns:
                # Calculate total missing files across all patterns
                total_missing = 0
                for pattern_key, pattern_data in patterns.items():
                    missing = find_missing_files(pattern_data)
                    total_missing += len(missing)

                # Only include if there are missing files
                if total_missing > 0:
                    results[dirpath] = {
                        'folder_name': os.path.basename(dirpath),
                        'folder_path': dirpath,
            ...
            APP_LOGGER.warning(f"Permission denied accessing: {dirpath}")
        except Exception as e:
            APP_LOGGER.error(f"Error scanning {dirpath}: {e}")

    return results


def create_placeholders_for_folder(dirpath: str, patterns_data: Dict) -> Tuple[int, int, List[str]]:
    """
    Create placeholder files for all patterns in a folder.

    Args:
        dirpath: Directory path
        patterns_data: Patterns data from detect_file_patterns

    Returns:
        (total_created, total_failed, error_messages)
    """
    total_created = 0
    total_failed = 0
    error_messages = []
    ...
    for pattern_key, pattern_data in patterns_data.items():
        ...
        if missing:
            success, message, created_files = create_placeholder_files(dirpath, pattern_data, missing)

            if success:
                total_created += len(created_files)
                # Log operation
                log_missing_files_operation(dirpath, pattern_key, pattern_data, missing, created_files)
            else:
                total_failed += len(missing)
                error_messages.append(f"{dirpath}: {message}")

    return total_created, total_failed, error_messages


# ==============================
# RECYCLE BIN FUNCTIONS (v7.2)
# ==============================
def send_files_to_recycle_bin(file_paths: List[str]) -> Tuple[bool, str, List[str], List[str]]:
    """
    Send files to recycle bin using send2trash.

    Args:
        file_paths: List of absolute file paths to send to recycle bin

    Returns:
        (success, message, succeeded_files, failed_files)
    """
    if not RECYCLE_BIN_AVAILABLE:
        return False, "send2trash library not installed", [], file_paths
    ...
    for filepath in file_paths:
        try:
            if not os.path.exists(filepath):
                APP_LOGGER.warning(f"File does not exist: {filepath}")
                failed.append(filepath)
                continue

            # Security check: validate path is not protected
            safe, msg = is_safe_directory(os.path.dirname(filepath))
            if not safe:
                APP_LOGGER.error(f"Attempted to delete from protected path: {filepath}")
                failed.append(filepath)
                continue
            ...
            APP_LOGGER.info(f"Sent to recycle bin: {filepath}")

        except Exception as e:
            APP_LOGGER.error(f"Failed to send to recycle bin: {filepath} - {e}")
            failed.append(filepath)

    if succeeded and not failed:
        return True, f"Successfully sent {len(succeeded)} file(s) to recycle bin", succeeded, failed
    elif succeeded and failed:
        return True, f"Sent {len(succeeded)} file(s) to recycle bin, {len(failed)} failed", succeeded, failed
    else:
        return False, f"Failed to send {len(failed)} file(s) to recycle bin", succeeded, failed


def log_recycle_bin_operation(file_paths: List[str], succeeded: List[str], failed: List[str]):
    """
    Log recycle bin operation for tracking and potential recovery.
    """
    log_entry = {
        "timestamp": datetime.now().isoformat(),
        "operation": "send_to_recycle_bin",
    ...
    try:
        with open(DATA_DIR.operations_file, 'a', encoding='utf-8') as f:
            f.write(json.dumps(log_entry) + '\n')
    except Exception as e:
        APP_LOGGER.error(f"Failed to log recycle bin operation: {e}")


def scan_missing_files_gui():
    """
    GUI wrapper for missing file scanner with mode selection (v7.2).
    Shows mode selection: single folder or recursive.
    """
    target_dir = (target_entry.get() or "").strip()
    ...
    # Ask user: single folder or recursive?
    mode_dialog = tk.Toplevel(root)
    mode_dialog.title("Missing File Scanner - Select Mode")
    mode_dialog.geometry("500x280")
    mode_dialog.transient(root)
    mode_dialog.grab_set()

    tk.Label(
        mode_dialog,
        text="Missing File Scanner",
        font=("Arial", 14, "bold")
    ).pack(pady=15)
    ...
    selected_mode = tk.StringVar(value="single")

    ttk.Radiobutton(
        mode_dialog,
        text="📂 Single Folder - Scan only this directory",
    ...
    ttk.Radiobutton(
        mode_dialog,
        text="🔄 Recursive - Scan all subfolders independently",
        variable=selected_mode,
        value="recursive"
    ).pack(anchor="w", padx=50, pady=5)
    ...
    def on_continue():
        ...
        if mode == "single":
            scan_single_folder_mode(target_dir)
        else:
            scan_recursive_folder_mode(target_dir)

    button_frame = ttk.Frame(mode_dialog)
    button_frame.pack(pady=20)

    ttk.Button(button_frame, text="Cancel", command=mode_dialog.destroy).pack(side="left", padx=5)
    ttk.Button(button_frame, text="Continue", command=on_continue).pack(side="left", padx=5)


def scan_single_folder_mode(target_dir: str):
    """
    Single-folder mode (original behavior).
    Detect patterns in one directory and show pattern selection.
    """
    # Detect patterns
    ...
    # Show pattern selection dialog (extracted from original code)
    show_single_folder_pattern_selection(target_dir, patterns)


def show_single_folder_pattern_selection(target_dir: str, patterns: Dict):
    """
    Show pattern selection dialog for single folder mode.
    This is the original pattern selection logic extracted from scan_missing_files_gui.
    """
...
def scan_recursive_folder_mode(target_dir: str):
    """
    Recursive mode - scan all subfolders independently.
    Shows progress dialog while scanning, then shows folder selection UI.
    """
    # Create progress dialog
    progress_dialog = tk.Toplevel(root)
    progress_dialog.title("Scanning Subfolders...")
    ...
    tk.Label(
        progress_dialog,
        text="Scanning subfolders for patterns...",
        font=("Arial", 12, "bold")
    ).pack(pady=10)

    progress_label = tk.Label(progress_dialog, text="Starting scan...", font=("Arial", 9))
    progress_label.pack(pady=5)

    progress_bar = ttk.Progressbar(progress_dialog, mode='determinate', length=500)
    progress_bar.pack(pady=10)

    folder_results = {}

    def progress_callback(current, total, folder_path):
        """Update progress UI"""
        progress_bar['maximum'] = total
        progress_bar['value'] = current
        relative_path = os.path.relpath(folder_path, target_dir) if folder_path != target_dir else "."
        progress_label.config(text=f"Scanning: {relative_path} ({current}/{total})")
        progress_dialog.update()

    # Run scan in thread to keep UI responsive
    def scan_thread():
        nonlocal folder_results
        try:
            folder_results = detect_file_patterns_recursive(target_dir, progress_callback)
        except Exception as e:
            APP_LOGGER.error(f"Error in recursive scan: {e}")
        finally:
            ...
            if not folder_results:
                messagebox.showinfo(
                    "No Patterns Found",
                    "No subfolders with missing files detected.\n\n"
                    "The scanner looks for numeric patterns and checks for gaps.",
                    parent=root
                )
            else:
                show_recursive_folder_selection(target_dir, folder_results)

    thread = threading.Thread(target=scan_thread, daemon=True)
    thread.start()


def show_recursive_folder_selection(parent_dir: str, folder_results: Dict):
    """
    Show selection UI for recursive scan results.
    User can select which subfolders to process.
    """
    dialog = tk.Toplevel(root)
    dialog.title("Missing File Scanner - Select Folders to Process")
    ...
    tk.Label(
        dialog,
        text=f"Found {len(folder_results)} subfolder(s) with missing files",
        font=("Arial", 14, "bold")
    ).pack(pady=10)

    tk.Label(
        dialog,
        text=f"Parent: {parent_dir}",
        font=("Arial", 9),
        fg="gray"
    ).pack()
    ...
    columns = ("Folder", "Patterns", "Missing Files", "Path")
    ...
    tree.heading("Missing Files", text="Missing Files")
    tree.heading("Path", text="Relative Path")

    tree.column("#0", width=50)
    tree.column("Folder", width=150)
    tree.column("Patterns", width=80)
    tree.column("Missing Files", width=100)
    ...
    folder_list = []
    for dirpath, data in sorted(folder_results.items()):
    ...
    def toggle_selection(event):
        ...
        if item:
            for folder in folder_list:
                if folder['item_id'] == item:
                    folder['selected'] = not folder['selected']
                    symbol = "☑" if folder['selected'] else "☐"
                    tree.item(item, text=symbol)
                    update_count()
                    break
    ...
    # Selection count
    count_var = tk.StringVar(value=f"Selected: 0 of {len(folder_results)}")
    count_label = tk.Label(dialog, textvariable=count_var, font=("Arial", 10, "bold"))
    count_label.pack(pady=5)

    def update_count():
        selected = sum(1 for f in folder_list if f['selected'])
        count_var.set(f"Selected: {selected} of {len(folder_results)}")

    def select_all():
        for folder in folder_list:
            folder['selected'] = True
            tree.item(folder['item_id'], text="☑")
        update_count()
    ...
    def process_selected():
        selected_folders = [f for f in folder_list if f['selected']]

        if not selected_folders:
            messagebox.showwarning("No Selection", "Please select folders to process.", parent=dialog)
            return

        # Confirmation
        total_missing = sum(f['data']['total_missing'] for f in selected_folders)
        confirm = messagebox.askyesno(
            "Confirm Creation",
            f"Create {total_missing} placeholder file(s) across {len(selected_folders)} folder(s)?",
            parent=dialog
        )
        ...
        for folder in selected_folders:
            created, failed, errors = create_placeholders_for_folder(
                folder['dirpath'],
                folder['data']['patterns']
            )
        ...
        if total_created > 0 and total_failed == 0:
            messagebox.showinfo(
                "Success",
                f"Successfully created {total_created} placeholder file(s)!",
                parent=dialog
            )
            dialog.destroy()
        elif total_created > 0:
            error_msg = f"Created {total_created} placeholder(s), {total_failed} failed.\n\nErrors:\n"
            error_msg += "\n".join(all_errors[:5])  # Show first 5 errors
            if len(all_errors) > 5:
                error_msg += f"\n... and {len(all_errors)-5} more"
            messagebox.showwarning("Partial Success", error_msg, parent=dialog)
        else:
            error_msg = f"Failed to create placeholders.\n\nErrors:\n"
            error_msg += "\n".join(all_errors[:5])
            if len(all_errors) > 5:
                error_msg += f"\n... and {len(all_errors)-5} more"
    ...
    ttk.Button(left_buttons, text="Select All", command=select_all).pack(side="left", padx=2)
    ttk.Button(left_buttons, text="Select None", command=select_none).pack(side="left", padx=2)

    right_buttons = ttk.Frame(button_frame)
    right_buttons.pack(side="right")

    ttk.Button(right_buttons, text="Cancel", command=dialog.destroy).pack(side="left", padx=2)
    ttk.Button(right_buttons, text="Create Placeholder Files", command=process_selected).pack(side="left", padx=2)
...
def search_and_collect():
    ...
    try:
        ...
        matching_files.sort(key=lambda x: normalize_case(x[1]))
...
def show_file_selection_window(source_dir, pattern, matching_files):
    ...
    def send_selected_to_recycle():
        """Send selected files to recycle bin with confirmation"""
        selected_files = [(filepath, filename) for filepath, filename, var in file_data if var.get()]

        if not selected_files:
            messagebox.showwarning("No Selection", "Please select files to send to recycle bin.", parent=popup)
            return

        if not RECYCLE_BIN_AVAILABLE:
            messagebox.showerror(
                "Feature Unavailable",
                "Recycle bin feature requires 'send2trash' library.\n\n"
        ...
        confirm = messagebox.askyesno(
            "Confirm Recycle Bin",
            f"Send {count} selected file(s) to recycle bin?\n\n"
            "Files can be restored from your system's recycle bin.",
            icon="warning",
            parent=popup
        )
        ...
        success, message, succeeded, failed = send_files_to_recycle_bin(file_paths)

        # Log operation
        if succeeded:
            log_recycle_bin_operation(file_paths, succeeded, failed)

        # Remove succeeded files from the list
        removed_indices = []
        for filepath in succeeded:
            for idx, (fp, fn, var) in enumerate(file_data):
                if fp == filepath:
                    removed_indices.append(idx)
                    break

        # Remove from UI (in reverse order to preserve indices)
        for idx in reversed(sorted(removed_indices)):
        ...
        # Show result
        if success:
            messagebox.showinfo("Recycle Bin", message, parent=popup)

            # If all files were sent, close the window
            if not file_data:
                messagebox.showinfo("Complete", "All files have been sent to recycle bin.\nClosing selection window.", parent=popup)
                popup.destroy()
        else:
            messagebox.showerror("Error", message, parent=popup)

    # Recycle bin button (v7.2)
    if RECYCLE_BIN_AVAILABLE:
        ttk.Button(
            move_frame,
            text="🗑️ Send to Recycle Bin",
            command=send_selected_to_recycle
...
def analyze_filename_patterns(filenames, progress_callback=None):
    ...
    for idx, filename in enumerate(filenames):
        ...
        flags = 0 if is_case_sensitive() else re.IGNORECASE
        m_camera = re.search(r'\b(IMG|DSC|DSCN|DCS|DCSN|VID|MOV|PXL)\b', base, flags)
        if m_camera:
            tag = m_camera.group(1) if is_case_sensitive() else m_camera.group(1).upper()
...
for title in sorted(sections.keys()):
    ...
    if title == "📁 Folder Tools":
        ...
        ttk.Label(pattern_row, text="(e.g., *filename*, name*, *.jpg)", foreground="gray", font=("Segoe UI", 8)).pack(side="left")
